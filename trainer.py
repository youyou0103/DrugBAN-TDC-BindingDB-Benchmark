...
class Trainer(object):
    ...
    def train(self):
        ...
        with open(log_path, 'w') as f:
            f.write("Epoch\tTime(s)\tTrain_Loss\tVal_Loss\tVal_AUC\tVal_AUPR\n")
        ...
        for i in range(self.epochs):
            ...
            if auroc >= self.best_auroc:
                ...
                counter = 0  # <--- 表现好，计数器归零
                print('Validation at Epoch ' + str(self.current_epoch) + ' ... (Best Model Saved)')
            else:
                counter += 1  # <--- 表现不好，计数器加一
                print(f'Validation at Epoch {self.current_epoch} ... (No Improvement: {counter}/{patience})')
            # ================================================================
            ...
            log_line = f"{self.current_epoch}\t{epoch_time:.2f}\t{train_loss:.6f}\t{val_loss:.6f}\t{auroc:.6f}\t{auprc:.6f}\n"
            with open(log_path, 'a') as f:
                f.write(log_line)

            # ================= [早停] C. 触发停止 =================
            if counter >= patience:
                print(f'\n[Early Stopping] Triggered! Stop training at epoch {self.current_epoch}.')
                break  # 跳出循环
            # ====================================================

        # --- 以下是训练循环结束后的测试逻辑 (这是你最关心的部分) ---

        # 这一步会用之前保存的 Best Model 去跑 Test Set
        ...
        # [日志] 5. 【新增】将最终 Test 结果追加写入到 txt 文件最底部
        with open(log_path, 'a') as f:
            f.write("\n" + "=" * 30 + "\n")
            f.write(f"FINAL TEST RESULT (Best Epoch: {self.best_epoch})\n")
            f.write("Test_AUC\tTest_AUPR\tF1\tSensitivity\tSpecificity\tAccuracy\tPrecision\n")
            f.write(
                f"{auroc:.6f}\t{auprc:.6f}\t{f1:.6f}\t{sensitivity:.6f}\t{specificity:.6f}\t{accuracy:.6f}\t{precision:.6f}\n")
            f.write("=" * 30 + "\n")
